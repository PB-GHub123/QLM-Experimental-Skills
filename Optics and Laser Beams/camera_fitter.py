...
class image:
    ...
    def openImage(self, fname, directory=None, fileType="bmp"):
        ...
        if directory != None:
            os.chdir(directory)
        if fileType == "bmp":
            self.imvals = np.array(Image.open(fname))[
                :, :, 0
            ]  # comment out this slice for other file formats
        elif fileType == "ascii":
    ...
    def cropImage(self, crop_x, crop_y):
        """Crop the image"""
        self.maxPixel()
        self.imvals = self.imvals[
            (self.yc - crop_y) : (self.yc + crop_y),
            (self.xc - crop_x) : (self.xc + crop_x),
        ]
    ...
    def fitImage(self):
        ...
        with open(filename_global+'.txt', "w") as fi:
            fi.write(filename_global+"\n")
            fi.write("wx =  "+str(self.gx.w0)+"\n")
            fi.write("ewx = "+str(self.gx.ew0)+"\n")
            fi.write("wy =  "+str(self.gy.w0)+"\n")
            fi.write("ewy = "+str(self.gy.ew0)+"\n")
...
class profile:
    ...
    def analyseBeamProfile(
        ...
        if z_offset != None:
            zarr = zarr + z_offset  # -9.5 - 2.

        zarr, wx, wy = arraySorter(zarr, wx, wy)
        print(zarr)

        plt.figure(figsize=(10, 4))
        plt.plot(zarr, wx, marker="o", linestyle="none", color="#AA2B4A")
        plt.plot(zarr, wy, marker="o", linestyle="none")
        # plt.xlim(6.5,9.)

        if fit == "hyperbola":
            self.fitHyperbola(zarr, wx, "x", clr="#AA2B4A")
            self.fitHyperbola(zarr, wy, "y", clr="#006388")
        elif fit == "linear":
            self.fitLinear(zarr, wx, "x", clr="#AA2B4A")
            self.fitLinear(zarr, wy, "y", clr="#006388")
    ...
    def fitHyperbola(self, x, y, xy, clr):
        ...
        if xy == "x":
            label = (
                "z0$_x$ = "
                + str(np.around(H.z0, 3))
                + ", w0$_x$ = "
                + str(np.around(H.w0, 3))
                + ", z$_{rx} = $"
                + str(np.around(H.zr, 3))
            )
        elif xy == "y":
            label = (
                "z0$_y$ = "
    ...
    def plotSingle(self, filename, single=True, crop_x=None, crop_y=None, angle=None):
        ...
        ax3.set_yticks(
            (
                0,
                np.around(np.around(np.max(im.xInt), -1) / 2),
                np.around(np.max(im.xInt), -1),
            )
        )
        ...
        ax1.imshow(
            im.imvals, interpolation="nearest", extent=[0, im.xpix[-1], 0, im.ypix[-1]]
        )
...
if __name__ == "__main__":
    ...
    imagePath = r"."  # Location of file or sequence of files to be fitted
    os.chdir(imagePath)
    plt.close("all")
    p = profile(
        imagePath, pixelSize=3.45e-3
    )  # Setup the class with the location of the images and the pixel size of the camera MAKE SURE THIS IS CORRECT!

    p.plotSingle(
        filename_global+'.png', crop_x=400, crop_y=300, angle=0
    )  # For fitting a single image. Crop the image around the max pixel value to help get a better fit

    # Z, waist_x, waist_y = p.analyseBeamProfile(
    #     "hyperbola", plot_all=True, angle=0
    # )  # Fit a range of images and extract the beam size. The filename is used as the Z coordinate
